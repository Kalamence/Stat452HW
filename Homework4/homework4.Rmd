...
Auto <- Auto %>% mutate(above_med = mpg > median,
                        above_med = above_med %>% as.numeric())
```
...
```{r}
tune.auto.linear <- tune(svm, above_med ~ . - mpg - name, data = Auto, kernal = "linear", ranges = list(cost = c(10^{-3:3})))

summary(tune.auto.linear)
tune.auto.linear

auto.linear <- svm(above_med ~. - mpg, data = Auto, kernal = "linear", cost = 1)
```
...
```{r}
tune.auto.rad <- tune(svm, above_med ~ . - mpg - name, data = Auto, kernal = "radial", 
...
auto.rad <- svm(above_med ~. - mpg, data = Auto, kernal = "radial", cost = 1, gamma = 0.5)

tune.auto.poly <- tune(svm, above_med ~ . - mpg - name, data = Auto, kernal = "polynomial", 
                         ranges = list(cost = c(10^{-3:3}),
                                       gamma = c(.5, 1:4)
                                       )
                       )
...
auto.poly <- svm(above_med ~. - mpg - name, data = Auto, kernal = "polynomial", cost = 1, gamma = 0.5)
...
Hint: In the lab, we used the plot() function for svm objects only in cases with p =2. Whenp>2, you can use the plot() function to create plots displaying pairs of variables at a time. Essentially, instead of typing
...
```{r}
plot(auto.linear, Auto, cylinders ~ origin)

plot(auto.rad, Auto, cylinders ~ origin)
