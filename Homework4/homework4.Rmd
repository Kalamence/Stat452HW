...
auto.linear <- svm(above_med ~., data = Auto, kernel = "linear", cost = 10)
```
...
```{r}
plot(auto.linear, data = Auto, cylinders~weight)
plot(auto.rad, Auto, acceleration ~ horsepower)

plot(auto.poly, Auto, cylinders ~ origin)
plot(auto.linear)
plot(auto.linear, Auto, horsepower ~ cylinders,
     slice = list(Acceleration = 6))
library(e1071)
plot(auto.linear, Auto[, -mpg], horsepower ~ cylinders)
```
