...
```{r}
tune.auto.linear <- tune(svm, above_med ~ . - mpg - name, data = Auto, kernel = "linear", 
                         ranges = list(cost = c(10^{-3:3})))
summary(tune.auto.linear)
tune.auto.linear

auto.linear <- svm(above_med ~. - mpg, data = Auto, kernel = "linear", cost = 1)
```
...
```{r}
tune.auto.rad <- tune(svm, above_med ~ . - mpg - name, data = Auto, kernel = "radial", 
                         ranges = list(cost = c(10^{-3:3}),
                                       gamma = c(.5, 1:4)
                                       )
                      )
...
auto.rad <- svm(above_med ~. - mpg, data = Auto, kernel = "radial", cost = 1, gamma = 0.5)

tune.auto.poly <- tune(svm, above_med ~ . - mpg - name, data = Auto, kernel = "polynomial", 
                         ranges = list(cost = c(10^{-3:3}),
                                       gamma = c(.5, 1:4)
                                       )
                       )
