...
for(j in 1:dim(data)[1]){
  for(i in 1:4){
    ...
    #print(USArrests[j,])
    comp[j, i] = sum(PCAScaled$rotation[,i]*PCAScaled$x[j,])^2
    
  }
}
comp
PCA$x[, 1]
firstComp = sum(comp[, 1]) / res
secondComp= sum(comp[, 2]) / res
thirdComp = sum(comp[, 3]) / res
fourthComp= sum(comp[, 4]) / res
```
